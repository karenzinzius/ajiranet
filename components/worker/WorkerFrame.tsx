    ...
    <div className="min-h-screen bg-[color:var(--bg)] text-[color:var(--text)]">
      ...
      <div className="flex flex-col lg:hidden min-h-screen">
        ...
        <nav className="fixed bottom-0 left-0 right-0 z-30 flex items-center justify-around border-t border-[color:var(--border)] bg-[color:var(--surface)] py-2 px-4">
          ...
          <Link href="/worker/notifications" className={`relative flex flex-col items-center gap-0.5 p-2 transition ${pathname === "/worker/notifications" ? "text-[color:var(--accent)]" : "text-[color:var(--muted)]"}`}>
            <svg className="h-6 w-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 01-3.46 0"/></svg>
            {unreadNotifications > 0 && <span className="absolute right-1 top-1 flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-[10px] font-bold text-white">{unreadNotifications}</span>}
