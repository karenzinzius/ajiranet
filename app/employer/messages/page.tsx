...
export default function EmployerMessagesPage() {
  ...
  const { employerMessages, markMessageRead } = useStore();
  const [selected, setSelected] = useState<string | null>(null);

  const open = employerMessages.find((m) => m.id === selected) ?? null;
  const unread = employerMessages.filter((m) => !m.read).length;

  const handleOpen = (id: string) => {
    setSelected(id);
    markMessageRead(id, "employer");
  };

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-[color:var(--text)]">{t.label}</h1>
        {unread > 0 && (
      ...
      {employerMessages.length === 0 ? (
        <div className="rounded-[2rem] border border-[color:var(--border)] bg-[color:var(--surface)] p-12 text-center space-y-3">
          ...
          <h2 className="font-bold text-[color:var(--text)]">{t.emptyTitle}</h2>
          <p className="text-sm text-[color:var(--muted)]">{t.emptySubtitle}</p>
        </div>
      ) : (
        <div className="grid gap-3 lg:grid-cols-[1fr_1.5fr] lg:items-start">
          {/* Message list */}
          <div className="space-y-2">
            {employerMessages.map((msg) => (
              <button
                key={msg.id}
